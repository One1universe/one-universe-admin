...
import { X, User, CheckCircle, XCircle, Loader2 } from "lucide-react";
...
const AdsSubscriberDetailsModal: React.FC<AdsSubscriberDetailsModalProps> = ({
  ...
  // Reset loading state and clear toasts when modal closes
  useEffect(() => {
    if (!isOpen) {
      setLoading(false);
      // Clear any active toasts when modal closes
  ...
  const handleSendReminder = async () => {
    console.log("📋 Full subscriber object:", subscriber);
    console.log("🆔 userId value:", subscriber?.userId);

    if (!subscriber?.userId || subscriber.userId.trim() === '') {
      showToast("error", "Error", "User ID not found in subscriber data");
      console.error("❌ User ID missing or empty from subscriber:", subscriber);
      return;
    }
    ...
    try {
      console.log("📤 Sending reminder to user:", subscriber.userId);
      ...
      showToast(
        "success",
        "Reminder Sent!",
        `Notification successfully sent to ${subscriber.sellerName}`
      );
